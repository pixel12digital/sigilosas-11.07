...
DROP FUNCTION IF EXISTS public.cadastrar_perfil_completo_de_acompanhante;
...
COMMENT ON FUNCTION public.cadastrar_perfil_completo_de_acompanhante(uuid, text, text, text, integer, text, integer, text, text, text, numeric, numeric, text, text, text, text, text, boolean, boolean, boolean, boolean, text, text, text, text, text, text, text, text[], text, jsonb)
IS 'Função transacional para realizar o cadastro completo de um perfil de acompanhante, incluindo dados principais e mídias. Garante atomicidade: ou tudo é inserido com sucesso, ou nada é alterado. Deve ser chamada após a criação do usuário em auth.users.'; 