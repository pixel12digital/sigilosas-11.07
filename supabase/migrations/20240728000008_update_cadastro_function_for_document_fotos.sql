-- Redefine a função de cadastro para alinhar com a nova estrutura de armazenamento de imagens.
-- A principal mudança é salvar as fotos de documentos na tabela 'fotos' com tipo 'documento'.

-- Drop a função antiga para garantir que a nova versão seja aplicada
DROP FUNCTION IF EXISTS public.cadastrar_perfil_completo_de_acompanhante(uuid, text, text, text, integer, text, uuid, text, text, text, numeric, numeric, text, text, text, text, text, boolean, boolean, boolean, boolean, text, text, text, text, text, text, text, text[], text, jsonb);

-- Recria a função com a lógica atualizada
...
BEGIN
    ...
    -- 6. Inserir os documentos na tabela correta 'documentos_acompanhante'
    IF p_documentos IS NOT NULL AND array_length(p_documentos, 1) > 0 THEN
        FOREACH doc_url IN ARRAY p_documentos
        LOOP
            -- Inserção na tabela correta, usando o tipo 'documento' que pode ser genérico aqui
            INSERT INTO public.documentos_acompanhante (acompanhante_id, url, storage_path, tipo, verificado)
            VALUES (p_user_id, doc_url, doc_url, 'documento', false);
        END LOOP;
    END IF;
...
COMMENT ON FUNCTION public.cadastrar_perfil_completo_de_acompanhante
IS 'Função transacional para cadastro completo. Salva documentos na tabela dedicada documentos_acompanhante.'; 